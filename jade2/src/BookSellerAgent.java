...
public class BookSellerAgent extends Agent {
  ...
  private record BookUnit(long time, int[] cost){}
	...
	addBehaviour(new TickerBehaviour(this, 1)
	  {
		  protected void onTick()
		  {
			  ...
			  {
				  Enumeration e2 = proposals.elements();
				  proposals.keySet().forEach((title) -> {
					  BookUnit entry = (BookUnit) e2.nextElement();
					  if(System.currentTimeMillis() - entry.time > 5000){
						  proposals.remove(title);
						  catalogue.put(title, entry.cost);
					  }
				  });
			  }
		  }
  ...
  public void updateCatalogue(final String title, int cost[]) {
    addBehaviour(new OneShotBehaviour() {
      public void action() {
		catalogue.put(title, cost);
		System.out.println(getAID().getLocalName() + ": " + title + " put into the catalogue. Price = " + cost[0] + " Shipping cost = "+ cost[1]);
      }
    } );
  }
	  ...
	  public void action() {
	    ...
	    if (msg != null) {
	      ...
	      if (price != null) {
	        ...
	        reply.setContent(String.valueOf(price[0] + price[1]));
			BookUnit bookUnit = new BookUnit(System.currentTimeMillis(), price);
			proposals.put(title, bookUnit);
	...
	private class PurchaseOrdersServer extends CyclicBehaviour {
	  public void action() {
	    ...
	    if (msg != null) {
	      String title = msg.getContent();
	      ACLMessage reply = msg.createReply();
	      BookUnit bookUnit = (BookUnit) proposals.remove(title);
